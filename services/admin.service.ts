...
export const adminService = {
  ...
  createCategory: (input: CategoryInput) =>
    apiRequest<Category>(endpoints.admin.categories, { method: "POST", body: input }),
  updateCategory: (id: string, input: Partial<CategoryInput & { isActive: boolean }>) =>
  ...
  users: (page = 1, limit = 10, search?: string) =>
    apiRequest<PaginatedData<User>>(paged(endpoints.admin.users, page, limit, search)),
  ...
  gear: (page = 1, limit = 10, search?: string) =>
    apiRequest<PaginatedData<GearItem>>(paged(endpoints.admin.gear, page, limit, search)),
  ...
  rentals: (page = 1, limit = 10) =>
    apiRequest<PaginatedData<RentalOrder>>(paged(endpoints.admin.rentals, page, limit)),
  payments: (page = 1, limit = 10) =>
    apiRequest<PaginatedData<Payment>>(paged(endpoints.admin.payments, page, limit)),
};
