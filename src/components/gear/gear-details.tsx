...
export function GearDetails({ id }: { id: string }) {
  ...
  if (query.isLoading) return <LoadingSpinner label="Loading gear details…" />;
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px]">
      <div>
        <div className="relative aspect-[16/10] overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-100 to-orange-100 dark:from-emerald-950 dark:to-orange-950">
          {image ? (
            <Image
              src={image}
              alt={gear.name}
        ...
        <div className="mt-7">
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-sm text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <ShieldCheck className="size-4 text-emerald-600" /> Provider:{" "}
              {gear.provider?.name ?? "Verified"}
            </span>
          </div>
