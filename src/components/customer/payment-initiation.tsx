...
export function PaymentInitiation({ rentalId }: { rentalId: string }) {
  const order = useQuery({
    ...
    queryFn: () => rentalService.detail(rentalId),
  });
  ...
  if (order.isError || !order.data)
    return <ErrorMessage message={getErrorMessage(order.error)} />;
  if (order.data.status !== "CONFIRMED")
    return (
      <ErrorMessage message="Payment is available only after the provider confirms this rental." />
    );
  return (
    <div className="max-w-lg rounded-2xl border bg-card p-6">
      <LockKeyhole className="size-9 text-primary" />
      <h2 className="mt-5 text-2xl font-bold">Secure SSLCommerz checkout</h2>
      <p className="mt-2 text-muted-foreground">
        You will be redirected to the payment gateway to complete this
        transaction.
      </p>
      <div className="my-6 flex justify-between border-y py-4">
        <span>Total due</span>
      ...
      >
        {initiate.isPending ? "Opening gateway…" : "Continue to payment"}
      </Button>
    </div>
  );
}
