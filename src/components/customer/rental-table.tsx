...
export function RentalTable({ limit = 20 }: { limit?: number }) {
  ...
  const query = useQuery({
    queryKey: [...queryKeys.rentals, { limit }],
    queryFn: () => rentalService.list({ page: 1, limit }),
  });
  const cancel = useMutation({
    mutationFn: rentalService.cancel,
    onSuccess: () => {
      toast.success("Rental cancelled");
      client.invalidateQueries({ queryKey: queryKeys.rentals });
    },
  ...
  if (!query.data?.items.length)
    return (
      <EmptyState
        ...
        action={
          <Link href="/gear" className="text-sm font-medium text-primary">
            Browse gear
          </Link>
        }
      />
            ...
            <tr key={order.id} className="border-t">
              <td className="p-4">
                ...
                <p className="text-xs text-muted-foreground">
                  {order.items?.[0]?.gearNameSnapshot ??
