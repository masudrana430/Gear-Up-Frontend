...
export function GearModerationTable() {
  ...
  const query = useQuery({
    queryKey: [...queryKeys.admin("gear"), { limit: 50 }],
    queryFn: () => adminService.gear(1, 50),
  });
  const update = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      adminService.setGearStatus(id, isActive),
    onSuccess: () => {
      toast.success("Gear status updated");
      client.invalidateQueries({ queryKey: queryKeys.admin("gear") });
    },
        ...
        <tbody>
          {query.data?.items.map((gear) => (
            <tr key={gear.id} className="border-t">
              ...
              <td className="p-4">{gear.provider?.name ?? gear.providerId}</td>
              ...
              <td className="p-4 text-right">
                <Button
                  size="sm"
                  variant={gear.isActive ? "outline" : "default"}
                  disabled={update.isPending}
                  onClick={() =>
                    update.mutate({ id: gear.id, isActive: !gear.isActive })
                  }
                >
                  {gear.isActive ? "Disable" : "Enable"}
                </Button>
              </td>
            </tr>
          ))}
